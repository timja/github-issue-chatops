...
import {addLabel, closeIssue, removeLabel, reopenIssue, requestReviewers, transferIssue} from "./app/github.js";
...
webhooks.on('issue_comment.created', async ({id, payload}) => {
  ...
  if (transferMatches) {
    ...
    await transferIssue(await getAuthToken(auth, payload.installation.id), payload.repository.owner.login, sourceRepo, targetRepo, payload.issue.node_id)
    return
  }
  ...
  if (closeMatches) {
    ...
    await closeIssue(await getAuthToken(auth, payload.installation.id), sourceRepo, payload.issue.node_id, reason)
    return
  }
  ...
  if (reopenMatches) {
    ...
    await reopenIssue(await getAuthToken(auth, payload.installation.id), sourceRepo, payload.issue.node_id)
    return
  }
  ...
  if (labelMatches) {
    ...
    await addLabel(await getAuthToken(auth, payload.installation.id), payload.repository.owner.login, sourceRepo, payload.issue.node_id, labels)
    return
  }
  ...
  if (removeLabelMatches) {
    ...
    await removeLabel(await getAuthToken(auth, payload.installation.id), payload.repository.owner.login, sourceRepo, payload.issue.node_id, labels)
    return
  }
  ...
  if (reviewerMatches) {
    ...
    await requestReviewers(await getAuthToken(auth, payload.installation.id), payload.repository.owner.login, sourceRepo, payload.issue.node_id, reviewers.users, reviewers.teams)
    return
  }
