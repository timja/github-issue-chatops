...
export async function addLabel(token, login, repository, labelableId, labels) {
  try {
    const {invalidLabels, labelIds} = await convertLabelsToIds(labels, token, login, repository);
...
export async function removeLabel(token, login, repository, labelableId, labels) {
  try {
    const {invalidLabels, labelIds} = await convertLabelsToIds(labels, token, login, repository);
...
export async function requestReviewers(token, organization, sourceRepo, issueId, users, teams) {
  try {
...
export async function transferIssue(token, owner, sourceRepo, targetRepo, issueId) {
  try {
